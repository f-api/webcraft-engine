...
/** Loopback-QA, outcome-only Spring H12f boundary. */
@RestController
public final class FinalSceneH12fController {
    public static final String PATH = "/worlds/{worldId}/debug/final-scene-h12f/execute";
    ...
    private final FinalSceneH12fOutcomeService outcomes;
    private final PlayerStore players;
    private final WorldStore worlds;
    private final SessionRegistry sessions;

    public FinalSceneH12fController(EngineProperties properties,
            FinalSceneH12fOutcomeService outcomes, PlayerStore players,
            WorldStore worlds, SessionRegistry sessions) {
        this.properties = Objects.requireNonNull(properties, "engine properties");
        this.outcomes = Objects.requireNonNull(outcomes, "H12f outcome service");
    ...
    @PostMapping(PATH)
    public ResponseEntity<OutcomeResponse> execute(@PathVariable long worldId,
        ...
        Outcome outcome = outcomes.execute(new FinalSceneH12fOutcomeService.ActiveBinding(
                worldId, world.getName(), input.nickname(), entry.connectionId()));
        return ResponseEntity.ok(OutcomeResponse.from(outcome));
    }
    ...
    private WorldAccess requireWorldAndPlayer(long worldId, Request input) {
        ...
        WorldAccess world = worlds.findById(worldId).orElseThrow(FinalSceneH12fController::invalidBody);
    ...
    private static Request readRequest(HttpServletRequest request) {
        ...
        if (!"H12f".equals(scenario) || !"spring".equals(authority)
    ...
    public record OrdinalResponse(int roleOrdinal, int candidateOrdinal) { }
    public record EvidenceResponse(String schema, List<OrdinalResponse> candidates) { }
    public record FailureResponse(Integer roleOrdinal, Integer candidateOrdinal, String code) { }
    public record OutcomeResponse(EvidenceResponse evidence, List<FailureResponse> failures,
            String cleanup, String operation, boolean evidenceAccepted) {
        static OutcomeResponse from(Outcome outcome) {
            EvidenceResponse evidence = outcome.evidence() == null ? null : new EvidenceResponse(
                    outcome.evidence().schema(), outcome.evidence().candidates().stream()
                            .map(row -> new OrdinalResponse(row.roleOrdinal(),
                                    row.candidateOrdinal())).toList());
            return new OutcomeResponse(evidence, outcome.failures().stream()
                    .map(row -> new FailureResponse(row.roleOrdinal(), row.candidateOrdinal(),
                            row.code().name())).toList(), outcome.cleanup().name(),
                    outcome.operation().name(), outcome.evidenceAccepted());
        }
    }
}
