...
/** Exact dormant executor for the pinned 26.3 {@code minecraft:shrt} piece only. */
public final class Mc263StrongholdRightTurnPieceExecutor {
    public static final String PIECE_TYPE = "minecraft:shrt";
    ...
    private Mc263StrongholdRightTurnPieceExecutor() { }
    ...
    public enum Orientation {
        ...
        public static Orientation fromNbtId(int id) {
            ...
            throw new IllegalArgumentException("stronghold-right-turn orientation outside 0..3");
        }
    }
    ...
    public record BoundingBox(int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
        public BoundingBox {
            if (minX > maxX || minY > maxY || minZ > maxZ) {
                throw new IllegalArgumentException("inverted stronghold-right-turn bounding box");
            }
        }
    ...
    public static ExecutionResult execute(PieceFacts piece, BoundingBox placementClip,
        ...
        int sideX = piece.orientation() == Orientation.NORTH
                || piece.orientation() == Orientation.EAST ? 4 : 0;
        for (int y = 1; y <= 3; y++) {
            for (int z = 1; z <= 3; z++) {
    ...
    static void preflight(PieceFacts piece, WorldAccess world) {
        ...
        if (!PIECE_TYPE.equals(piece.pieceType())) {
            throw new IllegalArgumentException("expected exact minecraft:shrt piece type");
        }
        if (piece.generationDepth() < 1 || piece.generationDepth() > 51) {
            throw new IllegalArgumentException("stronghold-right-turn depth outside 1..51");
        }
        ...
        if (xSpan != 5 || ySpan != 5 || zSpan != 5) {
            throw new IllegalArgumentException("stronghold-right-turn dimensions must be 5x5x5");
        }
        ...
        for (String state : requiredStates(piece)) {
            if (!world.supportsExactState(state)) {
                throw new UnsupportedOperationException("stronghold-right-turn state " + state);
            }
        }
