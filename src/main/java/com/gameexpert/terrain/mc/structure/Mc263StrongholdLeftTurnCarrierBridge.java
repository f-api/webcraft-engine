...
import com.gameexpert.terrain.mc.structure.Mc263StrongholdLeftTurnPieceExecutor.BoundingBox;
import com.gameexpert.terrain.mc.structure.Mc263StrongholdLeftTurnPieceExecutor.EntryDoor;
import com.gameexpert.terrain.mc.structure.Mc263StrongholdLeftTurnPieceExecutor.ExecutionResult;
import com.gameexpert.terrain.mc.structure.Mc263StrongholdLeftTurnPieceExecutor.Orientation;
import com.gameexpert.terrain.mc.structure.Mc263StrongholdLeftTurnPieceExecutor.PieceFacts;
...
/** Non-encodable dormant carrier bridge for exact pinned stronghold left-turn pieces. */
public final class Mc263StrongholdLeftTurnCarrierBridge {
    public static final String STRUCTURE = "minecraft:stronghold";

    private Mc263StrongholdLeftTurnCarrierBridge() { }
    ...
    public static List<PieceResult> execute(Mc263StructureCarrier carrier,
            ...
            Mc263StrongholdLeftTurnPieceExecutor.RandomSource random,
            Mc263StrongholdLeftTurnPieceExecutor.WorldAccess world) {
        ...
        for (ValidStart start : carrier.resolveStarts(references)) {
            ...
            for (int index = 0; index < start.orderedPieces().size(); index++) {
                ...
                if (!Mc263StrongholdLeftTurnPieceExecutor.PIECE_TYPE.equals(piece.pieceType())) {
                    continue;
                }
                PieceFacts facts = decodeFacts(piece);
                Mc263StrongholdLeftTurnPieceExecutor.preflight(facts, world);
        ...
        for (SelectedPiece piece : selected) {
            ...
            results.add(new PieceResult(piece.startKey(), piece.index(),
                    Mc263StrongholdLeftTurnPieceExecutor.execute(
    ...
    static PieceFacts decodeFacts(Piece piece) {
        if (piece.poolElement()
            ...
            throw new IllegalArgumentException("expected exact non-pool minecraft:shlt piece");
        }
        ...
        try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(payload))) {
            ...
            require(Mc263StrongholdLeftTurnPieceExecutor.PIECE_TYPE.equals(input.readUTF()));
            ...
            throw new IllegalArgumentException("noncanonical minecraft:shlt persisted NBT",
                    exception);
        }
    }
    ...
    static byte[] officialNbt(Mc263StructureCarrier.BoundingBox box, EntryDoor door,
        ...
        try {
            ...
            try (DataOutputStream output = new DataOutputStream(bytes)) {
                ...
                output.writeUTF(Mc263StrongholdLeftTurnPieceExecutor.PIECE_TYPE);
            ...
            throw new IllegalStateException("in-memory stronghold-left-turn NBT failed", exception);
        }
    }
    ...
    private static void require(boolean condition) {
        if (!condition) throw new IllegalStateException("noncanonical stronghold-left-turn NBT");
    }
